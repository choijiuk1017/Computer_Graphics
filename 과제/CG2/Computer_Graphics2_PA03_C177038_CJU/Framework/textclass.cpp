...
bool TextClass::SetCPU(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence1, cpuString, 10, 0, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetObjectNum(int object, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char objString[16];
	bool result;
	...
	result = UpdateSentence(m_sentence1, objString, 10, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolygonNum(int polygon, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char polyString[32];
	bool result;
	...
	result = UpdateSentence(m_sentence1, polyString, 10, 40, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenResolution(int width, int height, ID3D11DeviceContext* deviceContext)
{
	char widthString[16];
	char heightString[16];
	char resolutionString[48];
	bool result;
	...
	strcpy_s(resolutionString, "Resolution = ");
	strcat_s(resolutionString, widthString);
	strcat_s(resolutionString, " x ");
	strcat_s(resolutionString, heightString);

	// 텍스트 업데이트 (예시 위치: 오른쪽 위)
	result = UpdateSentence(m_sentence1, resolutionString, 10, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetBulletNum(int num, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence1, bulletNumString, 10, 80, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
