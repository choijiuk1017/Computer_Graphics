...
bool ModelClass::LoadModel(const WCHAR* filename)
{
	...
	std::wstring ext = (pos != std::wstring::npos) ? file.substr(pos + 1) : L"";

	if (ext == L"txt")
	{
		return LoadTextModel(filename); // 텍스트 파일 로드
	}
	else
	{
		return ReadFileCounts(filename); // OBJ 파일 로드
	}
...
bool ModelClass::LoadTextModel(const WCHAR* filename)
{
	ifstream fin;
	char input;
	int i;
