...
float4 LightPixelShader(PixelInputType input) : SV_TARGET
{
    ...
    spec1 = pow(saturate(dot(normalize(reflect(-normalize(input.lightPos1), input.normal)), input.viewDirection)), specularPower)
         * diffuseColorPoint[0];
    spec2 = pow(saturate(dot(normalize(reflect(-normalize(input.lightPos2), input.normal)), input.viewDirection)), specularPower)
        * diffuseColorPoint[1];
    spec3 = pow(saturate(dot(normalize(reflect(-normalize(input.lightPos3), input.normal)), input.viewDirection)), specularPower)
        * diffuseColorPoint[2];
    spec4 = pow(saturate(dot(normalize(reflect(-normalize(input.lightPos4), input.normal)), input.viewDirection)), specularPower)
         * diffuseColorPoint[3];
